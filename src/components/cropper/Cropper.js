...
export class WorkBench extends Component {
  ...
  render() {
    ...
    var img2Style = {
      ...imgSize,
      marginLeft: imgPos.left - cropRect.left,
      marginTop: imgPos.top - cropRect.top,
      transform: 'none'
    };
    ...
    return (
      <div className="cropper-container cropper-bg" style={benchSize}>
        ...
        <div
          className="cropper-drag-box cropper-modal cropper-move"
          data-action="move"
        />
        <div className="cropper-crop-box" style={cropRect}>
          <span className="cropper-view-box">
            <img src={imgsrc} style={img2Style} />
          </span>
          <span
            className={classNames('cropper-dashed dashed-h', {
              'cropper-hidden': !guides
            })}
          />
          <span
            className={classNames('cropper-dashed dashed-v', {
              'cropper-hidden': !guides
            })}
          />
...
export class Cropper extends Component {
  ...
  imgLoadOk() {
    ...
    if (this.benchRef) {
      var benchSize1 = {
        width: this.benchRef.clientWidth,
        height: this.benchRef.clientHeight
      };
      var benchSize = {
        width: this.benchRef.scrollWidth,
        height: this.benchRef.scrollHeight
      };
      var cropRect = {
        width: benchSize.width / 2,
        height: benchSize.height / 2,
        left: benchSize.width / 4,
        top: benchSize.height / 4
      };
      ...
      benchRect = {
        left: benchRect.left - bodyRect.left,
        top: benchRect.top - bodyRect.top,
  ...
  onWheel(e) {
    ...
    let imgCenterPos = {
      left: this.state.imgPos.left + this.state.imgSize.width / 2,
      top: this.state.imgPos.top + this.state.imgSize.height / 2
    };
    let aspectRatio =
      this.state.imgNaturalSize.width / this.state.imgNaturalSize.height;
    let imgSize = {
      width: this.state.imgSize.width * ratio,
      height: this.state.imgSize.width * ratio / aspectRatio
    };
    if (imgSize.width < 2) imgSize = { width: 2, height: 2 / aspectRatio };
    let imgPos = {
      left: imgCenterPos.left - imgSize.width / 2,
      top: imgCenterPos.top - imgSize.height / 2
    };
  ...
  cropStart(e) {
    ...
    } else action = element.getAttribute('data-action');
    var benchRect = this.state.benchRect;
    var mousePos = {
      left: e.pageX - benchRect.left,
      top: e.pageY - benchRect.top
    };
  ...
  cropMove(e) {
    ...
    switch (this.state.action) {
      case 'move':
        anchorPos = this.state.actionPos || {
        ...
        let imgPos = {
          left: this.state.imgPos.left + deltaX,
          top: this.state.imgPos.top + deltaY
        };
      ...
      case 'all':
        ...
        if (
          cropRect.left + cropRect.width + deltaX >
          this.state.benchSize.width
        )
          destRect.left = this.state.benchSize.width - cropRect.width;
        if (cropRect.top + deltaY < 0) destRect.top = 0;
        if (
          cropRect.top + cropRect.height + deltaY >
          this.state.benchSize.height
        )
          destRect.top = this.state.benchSize.height - cropRect.height;
      ...
      case ACTION_EAST:
        ...
        if (mousePos.left - anchorPos.left <= 0) {
          this.setState({
            ...
            cropRect: {
              ...cropRect,
              width: anchorPos.left - mousePos.left,
              left: mousePos.left
            }
          });
        } else {
          this.setState({
            cropRect: { ...cropRect, width: mousePos.left - anchorPos.left }
          });
        }
        break;
      case ACTION_WEST:
        ...
        if (anchorPos.left - mousePos.left <= 0) {
          this.setState({
            ...
            cropRect: {
              ...cropRect,
              width: mousePos.left - anchorPos.left,
              left: anchorPos.left
            }
          });
        } else {
      ...
      case ACTION_SOUTH:
        ...
        if (mousePos.top - anchorPos.top <= 0) {
          this.setState({
            ...
            cropRect: {
              ...cropRect,
              height: anchorPos.top - mousePos.top,
              top: mousePos.top
            }
          });
        } else {
          ...
          this.setState({
            cropRect: { ...cropRect, height: mousePos.top - anchorPos.top }
          });
        }
        break;
      case ACTION_NORTH:
        ...
        if (anchorPos.top - mousePos.top <= 0) {
          this.setState({
            ...
            cropRect: {
              ...cropRect,
              height: mousePos.top - anchorPos.top,
              top: anchorPos.top
            }
          });
        } else {
  ...
  getCroppedCanvas() {
    ...
    console.log(
      ...
      src.width,
      src.height,
      cropRect.left - imgPos.left,
      cropRect.top - imgPos.top,
      imgSize.width,
      imgSize.height
    );
    context.drawImage(
      ...
      src.width,
      src.height,
      imgPos.left - cropRect.left,
      imgPos.top - cropRect.top,
      imgSize.width,
      imgSize.height
    );
    return canvas;
  ...
  render() {
    ...
    return (
      <div
        {...otherProps}
        ref={v => {
          console.log('get div ref:', v);
          this.benchRef = v;
        }}
        onWheel={e => this.onWheel(e)}
        onMouseDown={e => this.cropStart(e)}
        onTouchStart={e => this.cropStart(e)}
        onTouchEnd={e => this.cropStop(e)}
        onTouchCancel={e => this.cropStop(e)}
        onMouseUp={e => this.cropStop(e)}
        onTouchMove={e => this.cropMove(e)}
        onMouseMove={e => this.cropMove(e)}
      >
        <img
          src={src}
          style={{
            width: '100%',
            opacity: 0.2,
            display: this.state.reset ? 'block' : 'none'
          }}
          onLoad={() => this.imgLoadOk()}
          onError={() => this.imgLoadFail()}
          ref={v => {
            console.log('get img ref:', v);
            this.imageRef = v;
          }}
        />
        ...
        {!this.state.reset && (
          <WorkBench
            src={src}
            guides={true}
            benchSize={this.state.benchSize}
            imgSize={this.state.imgSize}
            imgPos={this.state.imgPos}
            cropRect={this.state.cropRect}
            imgNaturalSize={this.state.imgNaturalSize}
          />
        )}
      </div>
    );
